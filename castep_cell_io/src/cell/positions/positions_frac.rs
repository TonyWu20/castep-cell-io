#![allow(dead_code)]
use castep_cell_fmt::{Cell, CellValue, ToCell, ToCellValue, FromCellValue, FromBlock, CResult, query::value_as_f64, query::value_as_str, query::value_as_string};
    ...
    fn from_cell_value(value: &CellValue<'_>) -> CResult<Self> {
        match value {
            CellValue::Array(arr) if arr.len() >= 4 => {
                ...
                //   "SPIN 1.0"       → separate tokens for keyword and value
                //   "SPIN= 1.0"      → "SPIN=" token, then value
                //   "SPIN=1.0"       → single token, value inline after =
                while idx < arr.len() {
                    ...
                    if keyword == "SPIN" {
                        ...
                        // Inline: "SPIN=1.0" — value is embedded after =
                        let val_str = &keyword["SPIN=".len()..];
                        spin = Some(val_str.parse::<f64>().map_err(|_| {
                            castep_cell_fmt::Error::Message(
                                format!("invalid SPIN value: {val_str}"),
                            )
                        })?);
                        idx += 1;
                    // --- MIXTURE qualifier ---
                        ...
                        if idx + 2 < arr.len() {
                            let mix_idx = parse_mixture_index(&arr[idx + 1])?;
                        ...
                        // Inline: "MIXTURE=1" — index embedded, weight in next token
                        let val_str = &keyword["MIXTURE=".len()..];
                        let mix_idx = val_str.parse::<u32>().map_err(|_| {
                            castep_cell_fmt::Error::Message(
                                format!("invalid MIXTURE index: {val_str}"),
                            )
                        })?;
                        if idx + 1 < arr.len() {
                            ...
                            return Err(castep_cell_fmt::Error::Message(
                                "MIXTURE qualifier requires weight after index".into(),
                            ));
                        }
                    } else {
                        return Err(castep_cell_fmt::Error::Message(
...
/// Helper: parse a mixture index from a CellValue.
pub(crate) fn parse_mixture_index(val: &CellValue<'_>) -> castep_cell_fmt::CResult<u32> {
    match val {
        CellValue::UInt(u) => Ok(*u),
        CellValue::Int(i) if *i >= 0 => Ok(*i as u32),
        _ => Err(castep_cell_fmt::Error::Message(
