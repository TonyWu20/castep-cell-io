...
/// Represents a single atom entry within the POSITIONS_ABS block.
///
...
impl FromCellValue for PositionAbsEntry {
    fn from_cell_value(value: &CellValue<'_>) -> CResult<Self> {
        match value {
            ...
            _ => Err(castep_cell_io::Error::Message(
                "PositionAbsEntry must be an array of at least 4 elements".into(),
            )),
        }
    }
}
...
/// Represents the POSITIONS_ABS block.
///
/// Contains a list of atomic positions in absolute (Cartesian) coordinates.
/// Format:
/// %BLOCK POSITIONS_ABS
/// [units]
...
impl FromBlock for PositionsAbs {
    const BLOCK_NAME: &'static str = "POSITIONS_ABS";

    fn from_block_rows(rows: &[CellValue<'_>]) -> CResult<Self> {
        if rows.is_empty() {
            return Ok(PositionsAbs {
                unit: None,
                positions: vec![],
            });
        }
        ...
        let positions = rows[data_start..]
            .iter()
            .map(PositionAbsEntry::from_cell_value)
            .collect::<CResult<Vec<_>>>()?;

        Ok(PositionsAbs { unit, positions })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_position_abs_entry_without_qualifiers() {
        ...
        let entry = PositionAbsEntry::from_cell_value(&val).unwrap();
    ...
    #[test]
    fn test_position_abs_entry_with_spin() {
        let val = CellValue::Array(vec![
            CellValue::Str("Fe"),
            CellValue::Float(1.5),
            CellValue::Float(2.5),
    ...
    #[test]
    fn test_position_abs_entry_with_mixture() {
        let val = CellValue::Array(vec![
            CellValue::Str("O"),
            CellValue::Float(0.5),
            CellValue::Float(0.5),
    ...
    #[test]
    fn test_position_abs_entry_with_spin_and_mixture() {
        let val = CellValue::Array(vec![
            CellValue::Str("Fe"),
            CellValue::Float(1.0),
            CellValue::Float(2.0),
    ...
    #[test]
    fn test_position_abs_entry_insufficient_elements() {
        let val = CellValue::Array(vec![
            CellValue::Str("Fe"),
            CellValue::Float(0.0),
            CellValue::Float(0.0),
        ]);
        assert!(PositionAbsEntry::from_cell_value(&val).is_err());
    }

    #[test]
    fn test_positions_abs_no_unit() {
        let rows = vec![
            CellValue::Array(vec![
                CellValue::Str("Fe"),
                CellValue::Float(0.0),
        ...
        let result = PositionsAbs::from_block_rows(&rows).unwrap();
        assert_eq!(result.unit, None);
        assert_eq!(result.positions.len(), 2);
    }

    #[test]
    fn test_positions_abs_with_unit_ang() {
        let rows = vec![
    ...
    #[test]
    fn test_positions_abs_with_unit_bohr() {
        let rows = vec![
    ...
    #[test]
    fn test_positions_abs_empty() {
        let result = PositionsAbs::from_block_rows(&[]).unwrap();
        assert_eq!(result.unit, None);
        assert_eq!(result.positions.len(), 0);
    }

    #[test]
    fn test_positions_abs_block_name() {
        assert_eq!(PositionsAbs::BLOCK_NAME, "POSITIONS_ABS");
    }

    #[test]
    fn test_positions_abs_to_cell_no_unit() {
        let positions = PositionsAbs {
            unit: None,
            positions: vec![PositionAbsEntry {
    ...
    #[test]
    fn test_positions_abs_to_cell_with_unit() {
        let positions = PositionsAbs {
    ...
    #[test]
    fn test_position_abs_entry_to_cell_value() {
        let entry = PositionAbsEntry {
            species: Species::from_cell_value(&CellValue::Str("Fe")).unwrap(),
            coord: [0.1, 0.2, 0.3],
            spin: Some(1.5),
            mixture: Some((1, 0.7)),
