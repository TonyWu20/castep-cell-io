...
mod tests {
    ...
    fn test_from_cell_value_true() {
        ...
        assert_eq!(PhononSumRule::from_cell_value(&val).unwrap().0, true);
    }

    #[test]
    fn test_from_cell_value_false() {
        let val = CellValue::Bool(false);
        assert_eq!(PhononSumRule::from_cell_value(&val).unwrap().0, false);
    }

    #[test]
    fn test_key_name() {
        assert_eq!(PhononSumRule::KEY_NAME, "PHONON_SUM_RULE");
    }
}
